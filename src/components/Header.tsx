...
const Header: React.FC = () => {
  ...
  const getAvatarColors = (address: string) => {
    ...
    const baseColors = [
      ['#3AABEE', '#229ED9', '#1E88E5', '#1976D2', '#1565C0'],
      ['#FF3366', '#FF4D7F', '#FF6699', '#FF80B2', '#FF99CC'],
      ['#FFD700', '#FFC107', '#FFB300', '#FFA000', '#FF8F00'],
      ['#00C853', '#00E676', '#69F0AE', '#B9F6CA', '#E8F5E9'],
                ...
                <nav className="flex flex-col space-y-6 flex-1 bg-black/95">
                  ...
                  <span className="text-[18px] font-medium relative " >
                    ...
                    {item.isActive && (
                        ...
                        {item.soon && (
                            <span className="text-sm font-medium px-3 py-1 rounded-full bg-[#2A2A2E]/80 text-white backdrop-blur-sm border border-[#2A2A2E]  bg-gradient-to-r from-[#FF3366] via-[#FF6B6B] to-[#FF3366] group-hover:from-[#FF6B6B] group-hover:via-[#FF3366] group-hover:to-[#FF6B6B] transition-all duration-500 bg-[length:200%_100%] animate-gradient">
                      Soon
                    </span>
                        )}
                      </Link>
