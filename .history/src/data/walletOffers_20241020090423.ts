...
const generateTokenData = (token: TonToken, seed: number, targetValue: number): { balance: number; address: string; value: number } => {
  ...
  // Используем фиксированную цену для каждого токена
  const tokenPrice = 1; // Предполагаем, что 1 токен = 1 USD для простоты
  ...
  for (let i = 0; i < 14; i++) {
    ...
    const newOffer: WalletOffer = {
      ...
      tokens: baseOffer.tokens.map(t => {
        const targetValue = baseOffer.priceUSD / 6; // Распределяем общую стоимость равномерно между токенами
        return {
          ...t,
