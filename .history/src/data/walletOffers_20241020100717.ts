  ...
  const tokens = tonTokens.slice(0, 6).map((token, index) => ({
    token: {
      ...
      logo: token.symbol === 'JETTON' ? '/jetton.jpg' :
            ...
            token.symbol === 'MERIDIAN' ? '/meridian.png' : // Изменено расширение с .webp на .png
            token.symbol === 'GRAM' ? '/gram.webp' :
            token.logo,
      name: token.symbol === 'BOLT' ? 'MERIDIAN' : token.name,
      symbol: token.symbol === 'BOLT' ? 'MERIDIAN' : token.symbol
    },
...
// Функция для содания псевдослучайного числа на основе строки
