  ...
  const tokens = tonTokens.slice(0, 6).map((token, index) => ({
    token: {
      ...
      logo: token.symbol === 'JETTON' ? '/jetton.jpg' :
            ...
            token.symbol === 'BOLT' ? '/mytonwallet.webp' : // Добавляем проверку для BOLT
...
export const debugTokens = () => {
  console.log('Debug: tonTokens', tonTokens);
  console.log('Debug: walletOffers[0].tokens', walletOffers[0].tokens);
};
