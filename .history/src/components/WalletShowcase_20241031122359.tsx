...
const WalletCard: React.FC<WalletCardProps> = ({ offer }) => {
  ...
  const getWalletStatus = (id: string) => {
    const statusMap: { [key: string]: { emoji: string; text: string; color: string } | null } = {
      'cryptobot': { emoji: '🍅', text: 'Fresh', color: 'text-red-400 bg-red-400/10' },
      'tonkeeper': { emoji: '🛡️', text: 'Secure', color: 'text-blue-400 bg-blue-400/10' },
      'tonspace': { emoji: '⚡️', text: 'Fast', color: 'text-yellow-400 bg-yellow-400/10' },
      'mytonwallet': { emoji: '💎', text: 'Premium', color: 'text-purple-400 bg-purple-400/10' },
      'tonhub': { emoji: '🚀', text: 'Powerful', color: 'text-green-400 bg-green-400/10' },
    };
