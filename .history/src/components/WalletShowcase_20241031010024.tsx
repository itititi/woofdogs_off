...
const WalletCard: React.FC<WalletCardProps> = ({ offer }) => {
  return (
    <Link href={`/wallet/${offer.id}`}>
      <div className="bg-[#141414] rounded-xl overflow-hidden shadow-lg p-4 sm:p-6 border border-[#2A2A2E] hover:border-[#3AABEE]/20 transition-all duration-300 h-[240px] sm:h-[300px] flex flex-col">
        {/* Header section */}
        <div className="flex items-center mb-3 sm:mb-4">
          <div className="w-12 h-12 sm:w-16 sm:h-16 rounded-[22%] overflow-hidden mr-3 shadow-lg">
            <Image 
              src={offer.icon} 
              alt={offer.name} 
              width={64} 
          ...
          <div className="flex-1 min-w-0">
            <h1 className="text-[20px] sm:text-[28px] font-bold titanium-gradient mb-2 truncate">{offer.name}</h1>
            <div className="flex items-center gap-2">
              <span className="text-[14px] sm:text-[16px] text-yellow-400 font-medium bg-yellow-400/10 px-2.5 sm:px-3 py-0.5 sm:py-1 rounded-full">
                {offer.priceRange}
              </span>
              {offer.isHot && (
                <span className="text-[14px] sm:text-[16px] text-orange-500 font-medium bg-orange-500/10 px-2.5 sm:px-3 py-0.5 sm:py-1 rounded-full animate-pulse">
                  🔥 Hot
                </span>
              )}
            </div>
        ...
        {/* Description */}
        <p className="text-[14px] sm:text-[16px] text-gray-300 mb-6 line-clamp-2 leading-relaxed">{offer.description}</p>

        {/* Price section */}
        <div className="bg-[#1A1A1A] rounded-2xl p-3 sm:p-4 mt-auto">
          <div className="flex flex-col sm:flex-col gap-2">
            <div className="flex items-baseline">
              <span className="text-[18px] sm:text-[24px] font-bold bg-gradient-to-r from-[#A0D8EF] to-[#3AABEE] text-transparent bg-clip-text">
                ${offer.priceUSD}
              </span>
            ...
            <div className="flex items-center justify-between">
              <div className="flex items-center">
                ...
                <span className="text-[14px] sm:text-[16px] font-semibold text-green-500 ml-1.5 sm:ml-2">${offer.auctionPriceUSD}</span>
              </div>
              <div className="flex items-center">
                <span className="text-[14px] text-gray-400">Buy:</span>
                <span className="text-[14px] sm:text-[16px] font-semibold text-yellow-500 ml-1.5 sm:ml-2">
      ...
      <div className="max-w-7xl mx-auto">
        {hotOffers.length > 0 && (
          <>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 px-4 sm:px-6 lg:px-8 mb-10">
        ...
        {regularOffers.length > 0 && (
          <>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 px-4 sm:px-6 lg:px-8">
