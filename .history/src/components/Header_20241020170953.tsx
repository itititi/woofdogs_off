          ...
          <div className="flex items-center justify-between h-16 xl:h-20">
            ...
            <div className="hidden lg:flex items-center space-x-6">
              <nav className="flex items-center space-x-6">
                ...
                >
                  {getLocalizedText('TON Wallets')}
                </Link>
                <span className="text-gray-400 cursor-not-allowed text-sm leading-tight whitespace-nowrap bg-[#1C1C1E] px-3 py-1 rounded-full">
                  {getLocalizedText('Ethereum Wallets')}
                  <span className="text-xs ml-1 bg-[#2A2A2E] text-gray-400 px-1 py-0.5 rounded-full leading-none">{getLocalizedText('Soon')}</span>
                </span>
                <span className="text-gray-400 cursor-not-allowed text-sm leading-tight whitespace-nowrap bg-[#1C1C1E] px-3 py-1 rounded-full">
                  {getLocalizedText('Solana Wallets')}
                  <span className="text-xs ml-1 bg-[#2A2A2E] text-gray-400 px-1 py-0.5 rounded-full leading-none">{getLocalizedText('Soon')}</span>
                </span>
                <a href="#" className="text-white hover:text-gray-300 transition-colors text-sm leading-tight whitespace-nowrap">{getLocalizedText('Support')}</a>
              </nav>
              <AirdropButton />
            </div>
          ...
          <div className="lg:hidden absolute top-full left-0 right-0 bg-black border-t border-gray-800">
            <div className="px-2 pt-2 pb-3 space-y-1">
              ...
              <span className="block px-3 py-2 rounded-md text-base font-medium text-gray-400 cursor-not-allowed bg-[#1C1C1E]">
                {getLocalizedText('Ethereum Wallets')}
                <span className="text-xs ml-1 bg-[#2A2A2E] text-gray-400 px-1 py-0.5 rounded-full">{getLocalizedText('Soon')}</span>
              </span>
              <span className="block px-3 py-2 rounded-md text-base font-medium text-gray-400 cursor-not-allowed bg-[#1C1C1E]">
                {getLocalizedText('Solana Wallets')}
                <span className="text-xs ml-1 bg-[#2A2A2E] text-gray-400 px-1 py-0.5 rounded-full">{getLocalizedText('Soon')}</span>
              </span>
              <a href="#" className="block px-3 py-2 rounded-md text-base font-medium text-white hover:text-gray-300" onClick={toggleMenu}>{getLocalizedText('Support')}</a>
            </div>
          </div>
        )}
      </header>
