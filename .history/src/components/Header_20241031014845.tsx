...
const Header: React.FC = () => {
  ...
  const renderWalletButton = () => {
    if (!mounted || isConnected === null) {
      return (
        <div className="bg-[#141414] text-white py-1.5 px-3 rounded-full text-[12px] font-medium h-7 w-[120px] border border-[#2A2A2E] flex items-center justify-center">
    ...
    if (isConnected) {
      return (
        <button 
          ...
          className="bg-[#141414] text-white py-1.5 px-3 rounded-full text-[12px] font-medium flex items-center hover:bg-[#2A2A2E] transition-colors duration-300 h-7 border border-[#2A2A2E]"
        >
          <Avatar
            size={16}
            name="EQD....abc"
    ...
    return (
      <button
        ...
        className="bg-[#31A6F5] text-white py-1.5 px-3 rounded-full text-[12px] font-medium hover:bg-[#2691D9] transition-colors duration-300 h-7"
      >
        Connect Wallet
      </button>
    );
                ...
                {navItems.map((item, index) => (
                  item.isButton ? (
                    <Link key={index} href={item.href}>
                      <button className="bg-gradient-to-r from-purple-500 via-pink-500 to-red-500 text-white py-1.5 px-3 rounded-full text-[11px] font-medium h-6 flex items-center justify-center hover:brightness-110 transition-all duration-300">
                    ...
                    >
                      {item.label}
                      {item.soon && <span className="ml-1 text-[10px] bg-[#2A2A2E] px-1.5 py-0.5 rounded">Soon</span>}
                    </Link>
                  )
                ))}
              </nav>
