...
const WalletCard: React.FC<WalletCardProps> = ({ offer }) => {
        ...
        <div className="bg-[#141414] rounded-xl overflow-hidden shadow-lg p-5 border border-[#2A2A2E] hover:border-[#3AABEE]/20 hover:shadow-[0_0_20px_rgba(58,171,238,0.1)] hover:bg-[#1A1A1A] transition-all duration-300 flex items-center group">
          ...
          <div className="flex items-center gap-8">
            <div className="flex items-center gap-3">
              ...
              {walletStatus && (
                <span className={`text-[14px] font-medium px-3 py-1.5 rounded-xl border ${walletStatus.color} border-current/20`}>
  ...
  return (
    <Link href={`/wallet/${offer.id}`}>
      <div className="bg-[#141414] rounded-xl overflow-hidden shadow-lg p-4 sm:p-6 border border-[#2A2A2E] hover:border-[#2A2A2E] hover:shadow-[0_0_20px_rgba(42,42,46,0.7)] hover:bg-[#1A1A1A] transition-all duration-300 h-[320px] sm:h-[340px] flex flex-col">
        {/* Header section */}
        <div className="flex items-center mb-4">
          <div className="relative w-16 h-[120px] sm:w-20 sm:h-[140px] rounded-xl overflow-hidden mr-3 shadow-lg">
            <div className="absolute inset-0 bg-gradient-to-br from-[#3AABEE]/10 to-transparent"></div>
            <Image 
              src={offer.icon} 
              alt={offer.name} 
              width={80} 
              ...
              className="object-cover w-full h-full" 
            />
            <div className="absolute bottom-0 left-0 right-0 h-1/3 bg-gradient-to-t from-[#141414] to-transparent"></div>
          </div>
          <div className="flex-1 min-w-0">
            ...
            <div className="flex items-center gap-2 overflow-x-auto no-scrollbar">
              <span className="text-[14px] text-yellow-400 font-medium bg-yellow-400/10 px-2.5 py-1 rounded-full flex-shrink-0">
                {offer.priceRange}
              </span>
              {offer.isHot && (
                <span className="text-[14px] text-orange-500 font-medium bg-orange-500/10 px-2.5 py-1 rounded-full animate-pulse flex-shrink-0">
                  🔥 Hot
                </span>
              )}
              {walletStatus && (
                <span className={`text-[14px] font-medium px-2.5 py-1 rounded-full ${walletStatus.color} flex-shrink-0`}>
                  {walletStatus.emoji} {walletStatus.text}
                </span>
              )}
            </div>
        ...
        <div className="flex gap-2 mb-4">
          <div className="flex-1 bg-[#1A1A1A] rounded-xl p-2.5">
            <span className="text-[12px] text-gray-400">Storage</span>
            <div className="flex items-center gap-1 mt-1">
              <span className="text-[14px] font-medium text-white">128GB</span>
              <span className="text-[12px] text-gray-400">Base</span>
            </div>
          </div>
          <div className="flex-1 bg-[#1A1A1A] rounded-xl p-2.5">
            <span className="text-[12px] text-gray-400">Colors</span>
            <div className="flex items-center gap-1 mt-1.5">
              <div className="w-3 h-3 rounded-full bg-[#3AABEE]"></div>
              <div className="w-3 h-3 rounded-full bg-[#2691D9]"></div>
        ...
        {/* Description */}
        <p className="text-[14px] text-gray-300 mb-4 line-clamp-2">
          {offer.description}
        </p>
        ...
        <div className="bg-[#1A1A1A] rounded-xl p-4 mt-auto">
          <div className="flex flex-col gap-3">
            <div className="flex items-baseline justify-between">
              ...
              <span className="text-[14px] text-gray-400">Final Price</span>
            </div>
            <div className="flex items-center justify-between">
              <div className="flex items-center">
                <span className="text-[14px] text-gray-400">Trade-in:</span>
                <span className="text-[16px] font-semibold text-green-500 ml-1.5">-${offer.auctionPriceUSD}</span>
              </div>
              <div className="flex items-center">
                <span className="text-[14px] text-gray-400">Monthly:</span>
                <span className="text-[16px] font-semibold text-yellow-500 ml-1.5">
                  ${Math.round(offer.priceUSD / 12)}
                </span>
              </div>
            </div>
          </div>
