                ...
                {navItems.map((item, index) => (
                  item.isButton ? (
                    <Link key={index} href={item.href}>
                      <button className="bg-gradient-to-r from-purple-500 via-pink-500 to-red-500 text-white py-2 px-4 rounded-full text-[14px] font-semibold h-9 flex items-center justify-center hover:brightness-110 transition-all duration-300">
                    ...
                    >
                      {item.label}
                      {item.soon && <span className="ml-1.5 text-[12px] bg-[#2A2A2E] px-1.5 py-0.5 rounded">Soon</span>}
                    </Link>
                  )
                ))}
              </nav>
