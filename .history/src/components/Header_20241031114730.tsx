...
const Header: React.FC = () => {
  ...
  const renderWalletButton = () => {
    if (!mounted || isConnected === null) {
      return (
        <div className={`bg-[#141414] text-white py-1.5 px-3 rounded-full text-[12px] font-medium h-7 w-[120px] flex items-center justify-center ${
          isMobileOrTablet ? 'border border-[#2A2A2E]' : ''
        }`}>
    ...
    if (isConnected) {
      return (
        <button 
          ...
          className={`bg-[#141414] text-white py-1.5 px-3 rounded-full text-[12px] font-medium flex items-center hover:bg-[#2A2A2E] transition-colors duration-300 h-7 ${
            isMobileOrTablet ? 'border border-[#2A2A2E]' : ''
          }`}
        >
          <Avatar
