...
const WalletCard: React.FC<WalletCardProps> = ({ offer }) => {
  ...
  const getWalletStatus = (id: string) => {
    const statusMap: { [key: string]: { emoji: string; text: string; color: string } | null } = {
      'wallet1': { emoji: '🍅', text: 'Fresh', color: 'text-red-400 bg-red-400/10' },
      'wallet2': { emoji: '🛡️', text: 'Secure', color: 'text-blue-400 bg-blue-400/10' },
      'wallet3': { emoji: '⚡️', text: 'Fast', color: 'text-yellow-400 bg-yellow-400/10' },
    };
