...
const Header: React.FC = () => {
  ...
  const renderWalletButton = () => {
    if (!mounted || isConnected === null) {
      return (
        <div className="bg-[#141414] text-white py-2 px-5 rounded-full text-[18px] font-medium h-10 w-[160px] flex items-center justify-center border border-[#2A2A2E]">
    ...
    if (isConnected) {
      return (
        <button 
          ...
          className="bg-[#141414] text-white py-2 px-5 rounded-full text-[18px] font-medium flex items-center hover:bg-[#2A2A2E] transition-colors duration-300 h-10 border border-[#2A2A2E]"
        >
          <Avatar
            size={18}
            name="EQD....abc"
    ...
    return (
      <button
        ...
        className="bg-[#31A6F5] text-white py-2 px-5 rounded-full text-[18px] font-medium hover:bg-[#2691D9] transition-colors duration-300 h-10 border border-[#2A2A2E] flex items-center justify-center"
      >
        Connect Wallet
      </button>
    );
        ...
        <div className="max-w-7xl mx-auto px-6 sm:px-8 lg:px-10">
          <div className="flex items-center justify-between h-16 sm:h-20">
            <Link href="/" className="flex items-center">
              <Image src="/woodogslogo.svg" alt="WooDogs Logo" width={32} height={32} className="mr-3 invert" />
                ...
                {navItems.map((item, index) => (
                  item.isButton ? (
                    <Link key={index} href={item.href}>
                      <button className="bg-gradient-to-r from-purple-500 via-pink-500 to-red-500 text-white py-2 px-5 rounded-full text-[18px] font-medium h-10 flex items-center justify-center hover:brightness-110 transition-all duration-300">
                    ...
                    >
                      {item.label}
                      {item.soon && <span className="ml-1.5 text-[12px] bg-[#2A2A2E] px-2 py-0.5 rounded">Soon</span>}
                    </Link>
                  )
                ))}
              </nav>
                  ...
                  {isConnected && isDropdownOpen && (
                    <div className="absolute right-0 mt-2 w-56 rounded-xl bg-[#141414] border border-[#2A2A2E] shadow-lg">
                      <div className="py-1">
                        <Link 
                          href="/profile" 
                          className="flex items-center px-4 py-2.5 text-[14px] text-gray-300 hover:bg-[#2A2A2E] hover:text-white"
                        >
              ...
              <div className="absolute bottom-full mb-2 right-0 w-56 rounded-xl bg-[#141414] border border-[#2A2A2E] shadow-lg">
                <div className="py-1">
                  <button 
                    ...
                    className="flex items-center w-full text-left px-4 py-2.5 text-[18px] text-gray-300 hover:bg-[#2A2A2E] hover:text-white"
                  >
                    <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5 mr-3" viewBox="0 0 20 20" fill="currentColor">
                      <path fillRule="evenodd" d="M10 9a3 3 0 100-6 3 3 0 000 6zm-7 9a7 7 0 1114 0H3z" clipRule="evenodd" />
                    </svg>
                  ...
                  <button
                    onClick={handleLogout}
                    className="flex items-center w-full text-left px-4 py-2.5 text-[18px] text-red-400 hover:bg-[#2A2A2E] hover:text-red-500"
                  >
                    <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5 mr-3" viewBox="0 0 20 20" fill="currentColor">
                      <path fillRule="evenodd" d="M3 3a1 1 0 00-1 1v12a1 1 0 102 0V4a1 1 0 00-1-1zm10.293 9.293a1 1 0 001.414 1.414l-3-3a1 1 0 000-1.414l3-3a1 1 0 10-1.414 1.414L14.586 9H7a1 1 0 100 2h7.586l-1.293 1.293z" clipRule="evenodd" />
                    </svg>
