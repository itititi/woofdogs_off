...
const WalletCard: React.FC<WalletCardProps> = ({ offer }) => {
  return (
    <Link href={`/wallet/${offer.id}`}>
      <div className="bg-[#141414] rounded-xl overflow-hidden shadow-lg p-4 border border-[#2A2A2E] hover:border-[#3AABEE]/20 transition-all duration-300 h-[200px] flex flex-col">
        {/* Header section */}
        <div className="flex items-center mb-3">
          <div className="w-12 h-12 rounded-[20%] overflow-hidden mr-3 shadow-lg">
            <Image src={offer.icon} alt={offer.name} width={48} height={48} className="object-cover" />
          </div>
          <div className="flex-1 min-w-0">
            <h1 className="text-xl font-bold titanium-gradient mb-1 truncate">{offer.name}</h1>
            <div className="flex items-center gap-2">
              <span className="text-sm text-yellow-400 font-medium bg-yellow-400/10 px-2 py-0.5 rounded-full">
                {offer.priceRange}
              </span>
              {offer.isHot && (
                <span className="text-sm text-orange-500 font-medium bg-orange-500/10 px-2 py-0.5 rounded-full animate-pulse">
                  🔥 Hot
                </span>
              )}
            </div>
        ...
        {/* Description */}
        <p className="text-sm text-gray-300 mb-auto line-clamp-2">{offer.description}</p>

        {/* Price section */}
        <div className="bg-[#1A1A1A] rounded-lg p-3 mt-3">
          <div className="flex items-center justify-between">
            <div className="flex items-baseline">
              <span className="text-lg font-bold bg-gradient-to-r from-[#A0D8EF] to-[#3AABEE] text-transparent bg-clip-text">
                ${offer.priceUSD}
              </span>
            ...
            <div className="flex items-center gap-3">
              <div className="flex items-center">
                ...
                <span className="text-sm font-semibold text-green-500 ml-1">${offer.auctionPriceUSD}</span>
              </div>
              <div className="flex items-center">
