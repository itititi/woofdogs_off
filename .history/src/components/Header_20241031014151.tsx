...
const Header: React.FC = () => {
  ...
  const renderWalletButton = () => {
    if (!mounted || isConnected === null) {
      return (
        <div className="bg-[#141414] text-white py-2 px-3 rounded-full text-[14px] font-semibold h-9 w-[140px] border border-[#2A2A2E] flex items-center justify-center">
          <div className="w-4 h-4 rounded-full border-2 border-[#3AABEE] border-t-transparent animate-spin"></div>
        </div>
      );
    }
    ...
    if (isConnected) {
      return (
        <button 
          ...
          className="bg-[#141414] text-white py-2 px-3 rounded-full text-[14px] font-semibold flex items-center hover:bg-[#2A2A2E] transition-colors duration-300 h-9 border border-[#2A2A2E]"
        >
          <Avatar
            size={20}
            name="EQD....abc"
    ...
    return (
      <button
        ...
        className="bg-[#31A6F5] text-white py-2 px-4 rounded-full text-[14px] font-semibold hover:bg-[#2691D9] transition-colors duration-300 h-9"
      >
        Connect Wallet
      </button>
    );
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between h-14 sm:h-16">
            <Link href="/" className="flex items-center">
              <Image src="/woodogslogo.svg" alt="WooDogs Logo" width={24} height={24} className="mr-2.5 invert" />
              <span className="font-bold text-[18px]">WooDogs</span>
            </Link>
            {!isMobileOrTablet && (
                ...
                {navItems.map((item, index) => (
                  item.isButton ? (
                    <Link key={index} href={item.href}>
                      <button className="bg-gradient-to-r from-purple-500 via-pink-500 to-red-500 text-white py-1.5 px-3 rounded-full text-[12px] font-semibold h-7 flex items-center justify-center hover:brightness-110 transition-all duration-300">
                    ...
                    >
                      {item.label}
                      {item.soon && <span className="ml-1 text-[11px] bg-[#2A2A2E] px-1.5 py-0.5 rounded">Soon</span>}
                    </Link>
                  )
                ))}
              </nav>
              ...
              {!isMobileOrTablet && (
                <div className="relative">
                  ...
                  {isConnected && isDropdownOpen && (
                    <div className="absolute right-0 mt-2 w-48 rounded-xl bg-[#141414] border border-[#2A2A2E] shadow-lg">
                      <div className="py-1">
                        <Link href="/profile" className="block px-4 py-2 text-[14px] text-gray-300 hover:bg-[#2A2A2E] hover:text-white">
                          Profile
                        </Link>
                        <Link href="/settings" className="block px-4 py-2 text-[14px] text-gray-300 hover:bg-[#2A2A2E] hover:text-white">
                          Settings
                        </Link>
                        <button
                          onClick={handleLogout}
                          className="block w-full text-left px-4 py-2 text-[14px] text-red-400 hover:bg-[#2A2A2E] hover:text-red-500"
                        >
                          Disconnect
                        </button>
                      </div>
              ...
              <div className="absolute bottom-full mb-2 right-0 w-56 rounded-xl bg-[#141414] border border-[#2A2A2E] shadow-lg">
                <div className="py-1">
                  <button 
                    ...
                    className="block w-full text-left px-4 py-2.5 text-[18px] text-gray-300 hover:bg-[#2A2A2E] hover:text-white"
                  >
                    Profile
                  </button>
                  <button 
