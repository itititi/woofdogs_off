      ...
      <div className="bg-[#141414] rounded-2xl overflow-hidden shadow-md transition-all duration-300 hover:shadow-xl flex flex-col h-full cursor-pointer border border-[#2A2A2E] hover:border-[#3AABEE]/20">
        <div className="p-4 sm:p-5 flex flex-col flex-grow">
          <div className="flex items-center mb-4">
            <div className="w-14 h-14 sm:w-16 sm:h-16 rounded-[22%] overflow-hidden mr-3 shadow-lg">
              <Image src={offer.icon} alt={offer.name} width={64} height={64} className="object-cover" />
            </div>
            <div className="flex-grow">
              <h3 className="text-[24px] sm:text-xl font-bold titanium-gradient mb-1">{offer.name}</h3>
              <div className="flex items-center gap-2">
                <span className="text-[18px] text-yellow-400 font-medium bg-yellow-400/10 px-3 py-1 rounded-full whitespace-nowrap">
                  {offer.priceRange}
                </span>
                {offer.isHot && (
                  <span className="text-[18px] text-orange-500 font-medium bg-orange-500/10 px-3 py-1 rounded-full animate-pulse whitespace-nowrap">
                    🔥 Hot
                  </span>
                )}
              </div>
          ...
          <p className="text-[18px] text-gray-400 mb-4 line-clamp-2">{offer.description}</p>
          <div className="mt-auto">
            <div className="flex justify-between items-end mb-3">
              <div>
                <span className="text-[28px] sm:text-2xl font-bold bg-gradient-to-r from-[#A0D8EF] to-[#3AABEE] text-transparent bg-clip-text">
                  ${offer.priceUSD}
                </span>
                <span className="text-[18px] text-gray-400 ml-1">Balance</span>
              </div>
              <div className="text-right">
                <span className="text-[20px] font-semibold text-green-500">${auctionPrice}</span>
                <span className="text-[18px] text-gray-400 ml-1">Auction</span>
              </div>
            </div>
            <div className="flex items-center justify-between bg-[#1A1A1A] rounded-lg px-4 py-3">
              <span className="text-[18px] text-gray-400">Bid ends in:</span>
