...
export default function WalletPage({ params }: WalletPageProps) {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      if (!walletData) {
        router.push('/404'); // Перенаправляем на страницу 404, если кошелек не найден
        return;
      }
  ...
  if (!wallet) {
    return null; // Это не должно произойти, так как мы перенаправляем на 404 выше
  }
                    ...
                    <tr key={index} className="border-t border-[#2A2A2E]">
                      <td className="py-4">
                        <div className="flex items-center">
                          ...
                          />
                          <div>
                            <p className="font-semibold">{tokenData.token.symbol}</p>
                            <p className="text-sm text-gray-400">{tokenData.token.name}</p>
                          </div>
                        </div>
                      </td>
                      <td className="py-4">{tokenData.balance.toLocaleString()} {tokenData.token.symbol}</td>
                      <td className="py-4 text-blue-400">{tokenData.address}</td>
                      <td className="py-4 text-right">${tokenData.value.toLocaleString()}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
