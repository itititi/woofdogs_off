...
const getRandomTokens = (tokens: TonToken[], count: number): TonToken[] => {
  const shuffled = tokens.sort(() => 0.5 - Math.random());
  return [tokens[0], ...shuffled.slice(1, count)];
};
...
export default function WalletPage({ params }: WalletPageProps) {
  ...
  const [walletTokens, setWalletTokens] = useState<TonToken[]>([]);
          ...
          <div className="bg-[#1C1C1E] rounded-lg overflow-hidden shadow-xl p-6 sm:p-8 mt-8 border border-[#2A2A2E]">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
              {walletTokens.map((token, index) => (
                <div key={index} className="flex items-center bg-[#2A2A2E] p-4 rounded-lg">
                  <Image
                    src={token.logo}
                    alt={token.name}
                    width={32}
