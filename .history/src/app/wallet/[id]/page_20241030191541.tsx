        ...
        <div className="max-w-3xl mx-auto">
          <div className="bg-[#141414] rounded-2xl overflow-hidden shadow-2xl p-4 sm:p-6 mb-6 border border-[#2A2A2E]">
            ...
            <div className="flex items-center mb-6">
              <div className="w-20 h-20 rounded-[22%] overflow-hidden mr-4 shadow-lg">
                <Image src={wallet.icon} alt={wallet.name} width={80} height={80} className="object-cover" />
              </div>
              <div>
                <h1 className="text-[32px] font-bold titanium-gradient mb-2">{wallet.name}</h1>
                <div className="flex items-center gap-2">
                  <span className="text-[18px] text-yellow-400 font-medium bg-yellow-400/10 px-3 py-1.5 rounded-full shadow-md">
                    {wallet.priceRange}
                  </span>
                  {wallet.isHot && (
                    <span className="text-[18px] text-orange-500 font-medium bg-orange-500/10 px-3 py-1.5 rounded-full animate-pulse">
                      🔥 Hot
                    </span>
                  )}
                </div>
            ...
            {/* Description */}
            <p className="text-[20px] text-gray-300 mb-6 leading-relaxed">{wallet.description}</p>
            
            {/* Price section */}
            <div className="bg-[#1A1A1A] rounded-xl p-4 sm:p-6 mb-6 shadow-inner">
              <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                <div>
                  <div className="flex items-baseline mb-2">
                    <span className="text-[32px] font-bold bg-gradient-to-r from-[#A0D8EF] to-[#3AABEE] text-transparent bg-clip-text">
                      ${wallet.priceUSD}
                    </span>
                    <span className="text-[18px] text-gray-400 ml-2">
                      ≈ {tonAmount} TON
                    </span>
                  </div>
                  <div className="flex items-center gap-4">
                    <div>
                      <span className="text-[18px] text-gray-400">Current bid:</span>
                      <span className="text-[20px] font-semibold text-green-500 ml-2">${wallet.auctionPriceUSD}</span>
                    </div>
                    <div>
                      <span className="text-[18px] text-gray-400">Buy now:</span>
                      <span className="text-[20px] font-semibold text-yellow-500 ml-2">${buyNowPrice}</span>
                    </div>
                  </div>
                </div>
                <div className="flex flex-col items-end">
                  <span className="text-[18px] text-gray-400 mb-1">
                    {isAuctionEnded ? 'Auction ended' : 'Auction ends in:'}
                  </span>
                  <span className="text-[24px] font-semibold text-[#3AABEE]">
                    {timeLeft}
                  </span>
                </div>
              </div>
            ...
            {/* Tokens section */}
            <div className="mb-6">
              <h3 className="text-[24px] font-bold titanium-gradient mb-4">Included Tokens</h3>
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                {wallet.tokens.map((tokenData, index) => (
                  <div key={index} className="bg-[#1A1A1A] rounded-lg p-3 flex items-center shadow-md">
                    ...
                    />
                    <span className="text-[18px] font-medium">{tokenData.token.symbol}</span>
                  </div>
                ))}
                <div className="bg-[#1A1A1A] rounded-lg p-3 flex items-center justify-center">
                  <span className="text-[18px] font-medium text-[#3AABEE]">
            ...
            <div className="flex flex-col gap-3">
              <button className="w-full bg-gradient-to-r from-[#FFD700] to-[#FFA500] text-black text-[20px] font-bold py-4 px-6 rounded-full hover:brightness-110 transition-all duration-300 shadow-lg hover:shadow-xl">
                Place Bid: ${wallet.auctionPriceUSD}
              </button>
              <button className="w-full bg-gradient-to-r from-[#3AABEE] to-[#1E90FF] text-white text-[20px] font-bold py-4 px-6 rounded-full hover:brightness-110 transition-all duration-300 shadow-lg hover:shadow-xl">
                Buy Now: ${buyNowPrice}
              </button>
            </div>
          </div>
          ...
          <div className="bg-[#141414] rounded-2xl overflow-hidden shadow-2xl p-4 sm:p-6 border border-[#2A2A2E]">
            <div className="flex items-center mb-6">
              <Image
                ...
                className="mr-3"
              />
              <h2 className="text-[28px] font-bold titanium-gradient">What You'll Get</h2>
            </div>
            <ul className="space-y-4">
              <li className="flex items-center">
                ...
                <span className="text-[20px]">{walletContents}</span>
              </li>
              <li className="flex items-center">
                <svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6 mr-3 text-[#3AABEE]" viewBox="0 0 20 20" fill="currentColor">
