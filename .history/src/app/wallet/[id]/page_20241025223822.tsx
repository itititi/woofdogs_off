...
export default function WalletPage({ params }: WalletPageProps) {
  ...
  return (
    <div className="min-h-screen bg-black text-white flex flex-col">
      <Header />
      <main className="flex-grow py-4 px-4 mt-16 sm:mt-20 lg:mt-24 overflow-y-auto">
        <div className="max-w-4xl mx-auto">
          <div className="bg-[#141414] rounded-2xl overflow-hidden shadow-2xl p-4 sm:p-6 lg:p-8 mb-6 border border-[#2A2A2E] relative">
            <button 
              onClick={handleClose}
            ...
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-6">
              <div className="flex items-center mb-4 sm:mb-0">
                <div className="w-20 h-20 sm:w-24 sm:h-24 rounded-[22%] overflow-hidden mr-4 shadow-lg">
                  <Image src={wallet.icon} alt={wallet.name} width={96} height={96} className="object-cover" />
                </div>
                <h1 className="text-2xl sm:text-3xl lg:text-4xl font-bold titanium-gradient">{wallet.name}</h1>
              </div>
              <div className="flex flex-col sm:items-end">
                <span className="text-yellow-400 font-semibold text-sm sm:text-base bg-yellow-400/10 px-3 py-1 rounded-full shadow-md mb-2">
                  {wallet.priceRange}
                </span>
                {wallet.isHot ? (
                  <span className="text-orange-500 font-semibold text-sm sm:text-base bg-orange-500/10 px-3 py-1 rounded-full shadow-md animate-pulse">
                    🔥 Hot
                  </span>
                ) : (
                  <span className="text-green-500 font-semibold text-sm sm:text-base bg-green-500/10 px-3 py-1 rounded-full shadow-md">
                    🐸 Cool
                  </span>
                )}
              </div>
            ...
            <div className="flex items-center text-gray-300 mb-4">
              ...
              <span className="text-sm sm:text-base">Created on {wallet.createdAt}</span>
            </div>
            
            <p className="text-gray-300 text-base sm:text-lg mb-6 leading-relaxed">{wallet.description}</p>
            
            <div className="flex flex-col lg:flex-row lg:justify-between lg:items-stretch mb-6 bg-[#1A1A1A] p-4 sm:p-6 rounded-lg shadow-inner">
              <div className="flex-1 mb-4 lg:mb-0 lg:mr-6">
                <div className="mb-4 pb-4 border-b border-gray-700">
                  <span className="text-sm sm:text-base text-gray-400">Wallet Balance</span>
                  <div className="flex items-baseline">
                    <span className="text-3xl sm:text-4xl font-bold titanium-gradient">${wallet.priceUSD}</span>
                    <span className="text-lg sm:text-xl text-gray-300 ml-2">≈ {tonAmount} TON</span>
                  </div>
                </div>
                <div className="mb-4 pb-4 border-b border-gray-700">
                  <span className="text-sm sm:text-base text-gray-400">Current Bid</span>
                  <div className="flex items-baseline">
                    <span className="text-2xl sm:text-3xl font-semibold text-green-500">${bidPrice}</span>
                ...
                <div>
                  <span className="text-sm sm:text-base text-gray-400">Buy Now</span>
                  <div className="flex items-baseline">
                    <span className="text-2xl sm:text-3xl font-semibold text-yellow-500">${blitzPrice}</span>
              ...
              <div className="flex flex-col justify-center items-center lg:items-end bg-[#242424] p-4 rounded-lg">
                <span className="text-gray-400 text-sm sm:text-base mb-2">
                  {isAuctionEnded ? 'Auction ended' : 'Auction ends in'}
                </span>
                <span className="font-bold text-2xl sm:text-3xl text-[#FFA500]">
                  {timeLeft}
                </span>
              </div>
            </div>
            ...
            {/* Tokens list */}
            <div className="mb-6">
              <h3 className="text-xl sm:text-2xl font-bold titanium-gradient mb-4">Included Tokens</h3>
              <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3 sm:gap-4">
                {wallet.tokens.map((tokenData, index) => (
                  <div key={index} className="bg-[#1A1A1A] rounded-lg p-3 flex items-center shadow-md">
                    <Image
                    ...
                    />
                    <span className="font-medium text-sm sm:text-base">{tokenData.token.symbol}</span>
                  </div>
                ))}
                <div className="bg-[#1A1A1A] rounded-lg p-3 flex items-center justify-center shadow-md">
                  <span className="font-medium text-sm sm:text-base text-[#3AABEE]">
            ...
            <div className="flex flex-col sm:flex-row gap-4 mb-6">
              <button 
                className="flex-1 bg-gradient-to-r from-[#FFD700] to-[#FFA500] text-[#8B4513] text-lg sm:text-xl font-bold py-4 px-6 rounded-full hover:brightness-110 transition-all duration-300 shadow-lg hover:shadow-xl"
              >
                Place Bid: ${bidPrice ?? wallet.priceUSD} 
                {bidPrice && `(Save $${(wallet.priceUSD - bidPrice).toFixed(2)})`}
              </button>
              <button 
                className="flex-1 bg-gradient-to-r from-[#3AABEE] to-[#1E90FF] text-white text-lg sm:text-xl font-bold py-4 px-6 rounded-full hover:brightness-110 transition-all duration-300 shadow-lg hover:shadow-xl"
              >
