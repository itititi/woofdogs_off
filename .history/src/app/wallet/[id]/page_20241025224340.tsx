      ...
      <main className="flex-grow py-4 px-4 mt-16 overflow-y-auto">
        <div className="max-w-3xl mx-auto">
          <div className="bg-[#141414] rounded-2xl overflow-hidden shadow-2xl p-4 mb-4 border border-[#2A2A2E] relative">
            ...
            <div className="flex flex-col sm:flex-row sm:justify-between sm:items-stretch mb-6 bg-[#1A1A1A] p-4 rounded-lg shadow-inner">
              <div className="mb-2 sm:mb-0 flex-1">
                <div className="flex justify-between mb-2">
                  <span className="text-sm sm:text-base text-gray-300">Wallet Balance:</span>
                  <span className="text-sm sm:text-base text-gray-300">≈ {tonAmount} TON</span>
                </div>
                <div className="text-2xl sm:text-3xl font-bold titanium-gradient text-center mb-4">
                  ${wallet.priceUSD}
                </div>
                <div className="flex justify-between items-center mb-2">
                  <span className="text-sm sm:text-base text-gray-300">Current Bid:</span>
                  <div className="text-right">
                    <span className="text-lg sm:text-xl font-semibold text-green-500">${bidPrice ?? 'N/A'}</span>
                    {bidPrice !== null && wallet && (
                      <div className="text-sm text-green-500">
                        (Save ${(bidPrice - wallet.priceUSD).toFixed(2)})
                      </div>
                    )}
                  </div>
                </div>
                <div className="flex justify-between items-center">
                  ...
                  <div className="text-right">
                    <span className="text-lg sm:text-xl font-semibold text-yellow-500">${blitzPrice ?? 'N/A'}</span>
                    {blitzPrice !== null && wallet && (
              ...
              <div className="flex flex-col items-center sm:items-end sm:ml-4 mt-4 sm:mt-0 justify-center">
                <span className="text-gray-300 text-sm sm:text-base">
                  {isAuctionEnded ? 'Auction ended' : 'Auction ends in:'}
                </span>
                <span className="font-semibold text-xl sm:text-2xl text-[#FFA500] mt-2">
                  {timeLeft}
                </span>
              </div>
            </div>
            ...
            <div className="flex flex-col gap-3 mb-4">
              ...
              >
                Place Bid: ${bidPrice ?? 'N/A'} {bidPrice !== null && wallet && `(Save $${(wallet.priceUSD - bidPrice).toFixed(2)})`}
              </button>
              <button 
