...
interface TokenBalance extends TonToken {
  balance: number;
  address: string;
  value: number;
}

// Функция для генерации псевдослучайного числа на основе строки
...
const getRandomTokens = (tokens: TonToken[], count: number, walletId: string): TokenBalance[] => {
  const random = seededRandom(walletId);
  const shuffled = [...tokens].sort(() => random() - 0.5);
  return [tokens[0], ...shuffled.slice(1, count)].map(token => {
    const balance = parseFloat((random() / 4294967295 * 10000).toFixed(2));
    const value = parseFloat((random() / 4294967295 * 100).toFixed(2));
    const address = `EQ${Array.from({length: 20}, () => '0123456789ABCDEF'[random() % 16]).join('')}`;
    return {
      ...token,
      balance,
      address,
...
export default function WalletPage({ params }: WalletPageProps) {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      // Генерируем токены на основе ID кошелька
      const randomTokens = getRandomTokens(tonTokens, 6, params.id);
      setWalletTokens(randomTokens);
      ...
      <main className="flex-grow py-8 px-4 sm:py-12 sm:px-6 lg:px-8 mt-16">
        <div className="max-w-4xl mx-auto">
          ...
          <div className="bg-[#1C1C1E] rounded-lg overflow-hidden shadow-xl p-6 sm:p-8 mt-8 border border-[#2A2A2E]">
            <h2 className="text-2xl font-bold mb-6 bg-gradient-to-r from-[#2AABEE] to-[#1E88E5] text-transparent bg-clip-text">Tokens in this Wallet</h2>
              ...
              <table className="w-full text-left">
                <thead>
                  <tr className="text-gray-400 text-sm">
                    ...
                    <th className="pb-2">Token address</th>
                ...
                <tbody>
                  {walletTokens.map((token, index) => (
                    <tr key={index} className="border-t border-[#2A2A2E]">
                      ...
                      <td className="py-4">{token.balance.toLocaleString()} {token.symbol}</td>
                      <td className="py-4 text-blue-400">{token.address}</td>
                      <td className="py-4 text-right">${token.value.toLocaleString()}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
