        ...
        <div className="max-w-3xl mx-auto">
          <div className="bg-[#141414] rounded-2xl overflow-hidden shadow-2xl p-4 sm:p-6 mb-6 border border-[#2A2A2E]">
            ...
            <div className="flex items-center mb-6">
              <div className="w-16 h-16 sm:w-20 sm:h-20 rounded-[22%] overflow-hidden mr-4 sm:mr-6 shadow-lg">
                <Image src={wallet.icon} alt={wallet.name} width={80} height={80} className="object-cover" />
              </div>
              <div>
                <h1 className="text-2xl sm:text-3xl font-bold titanium-gradient mb-2">{wallet.name}</h1>
                <div className="flex items-center gap-2">
                  <span className="text-xs sm:text-sm text-yellow-400 font-medium bg-yellow-400/10 px-2 py-1 rounded-full shadow-md">
                    {wallet.priceRange}
                  </span>
                  {wallet.isHot && (
                    <span className="text-xs sm:text-sm text-orange-500 font-medium bg-orange-500/10 px-2 py-1 rounded-full animate-pulse">
                      🔥 Hot
                    </span>
                  )}
                </div>
            ...
            {/* Description */}
            <p className="text-sm sm:text-base text-gray-300 mb-6 leading-relaxed">{wallet.description}</p>
              ...
              <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                <div>
                  <div className="flex items-baseline mb-2">
                    <span className="text-2xl sm:text-3xl md:text-4xl font-bold bg-gradient-to-r from-[#A0D8EF] to-[#3AABEE] text-transparent bg-clip-text">
                      ${wallet.priceUSD}
                    </span>
                  ...
                  <div className="flex items-center gap-4">
                    <div>
                      ...
                      <span className="text-sm sm:text-base font-semibold text-green-500 ml-2">${wallet.auctionPriceUSD}</span>
                    </div>
                    <div>
                      <span className="text-xs sm:text-sm text-gray-400">Buy now:</span>
                      <span className="text-sm sm:text-base font-semibold text-yellow-500 ml-2">${buyNowPrice}</span>
                    </div>
                  </div>
                </div>
                <div className="flex flex-col items-end">
                  ...
                  <span className="text-base sm:text-lg font-semibold text-[#3AABEE]">
                    {timeLeft}
                  </span>
                </div>
              </div>
            ...
            {/* Tokens section */}
            <div className="mb-6">
              <h3 className="text-lg sm:text-xl font-bold titanium-gradient mb-4">Included Tokens</h3>
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                {wallet.tokens.map((tokenData, index) => (
                  <div key={index} className="bg-[#1A1A1A] rounded-lg p-3 flex items-center shadow-md">
                    ...
                    />
                    <span className="text-xs sm:text-sm font-medium">{tokenData.token.symbol}</span>
                  </div>
                ))}
                <div className="bg-[#1A1A1A] rounded-lg p-3 flex items-center justify-center">
                  <span className="text-xs sm:text-sm font-medium text-[#3AABEE]">
            ...
            <div className="flex flex-col gap-3">
              <button className="w-full bg-gradient-to-r from-[#FFD700] to-[#FFA500] text-black text-sm sm:text-base font-bold py-3 px-6 rounded-full hover:brightness-110 transition-all duration-300 shadow-lg hover:shadow-xl">
                Place Bid: ${wallet.auctionPriceUSD}
              </button>
              <button className="w-full bg-gradient-to-r from-[#3AABEE] to-[#1E90FF] text-white text-sm sm:text-base font-bold py-3 px-6 rounded-full hover:brightness-110 transition-all duration-300 shadow-lg hover:shadow-xl">
                Buy Now: ${buyNowPrice}
              </button>
            </div>
          </div>
