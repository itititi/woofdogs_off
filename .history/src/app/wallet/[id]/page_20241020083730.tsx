...
const getRandomTokens = (tokens: TonToken[], count: number): TokenBalance[] => {
  ...
  return [tokens[0], ...shuffled.slice(1, count)].map(token => ({
    ...
    address: `EQ${Math.random().toString(36).substr(2, 10)}...${Math.random().toString(36).substr(2, 10)}`,
...
export default function WalletPage({ params }: WalletPageProps) {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      // Выбираем случайные токены для кошелька (включая TON)
      const randomTokens = getRandomTokens(tonTokens, 6); // Выбираем 6 токенов, включая TON
