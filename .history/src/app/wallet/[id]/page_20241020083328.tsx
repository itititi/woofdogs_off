...
const generateTokenBalances = (totalValue: number, tokens: TonToken[]): TokenBalance[] => {
  console.log('Total value:', totalValue); // Отладочный вывод
  ...
  return shuffledTokens.map((token, index) => {
    ...
    if (isLast) {
      ...
      balance = Math.min(balance, remainingValue);
    }
    
    balance = parseFloat(balance.toFixed(2)); // Округляем до двух знаков после запятой
    remainingValue = Math.max(0, parseFloat((remainingValue - balance).toFixed(2))); // Округляем оставшееся значение
    
    console.log(`Token: ${token.symbol}, Balance: ${balance}`); // Отладочный вывод
...
export default function WalletPage({ params }: WalletPageProps) {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      const totalValue = parseFloat((Math.random() * (maxPrice - minPrice) + minPrice).toFixed(2));
      console.log('Generated total value:', totalValue); // Отладочный вывод
          ...
          <div className="bg-[#1C1C1E] rounded-lg overflow-hidden shadow-xl p-6 sm:p-8 mt-8 border border-[#2A2A2E]">
            ...
            <div className="mt-6 text-right">
              <p className="text-lg font-semibold">
                Total Value: $
                {tokenBalances.reduce((sum, tb) => sum + tb.balance, 0).toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
              </p>
            </div>
          </div>
        </div>
