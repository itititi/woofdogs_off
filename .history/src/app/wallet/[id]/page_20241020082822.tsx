...
export default function WalletPage({ params }: WalletPageProps) {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      // Генерируем случайные балансы для 10 случайных токенов
      const randomTokens = tonTokens.sort(() => 0.5 - Math.random()).slice(0, 10);
      const balances = randomTokens.map(token => ({
        token,
    ...
    <div className="min-h-screen bg-black text-white flex flex-col">
      ...
      <main className="flex-grow py-8 px-4 sm:py-12 sm:px-6 lg:px-8 mt-16">
        <div className="max-w-4xl mx-auto">
          ...
          {/* Добавьте новую секцию для отображения токенов */}
