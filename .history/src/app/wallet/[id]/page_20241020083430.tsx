...
const generateTokenBalances = (totalValue: number, tokens: TonToken[]): TokenBalance[] => {
  console.log('Generating token balances for total value:', totalValue);
  ...
  return shuffledTokens.map((token, index) => {
    ...
    if (isLast) {
      ...
      balance = Math.random() * (remainingValue / 2);
    }
    
    balance = Math.min(balance, remainingValue);
    balance = parseFloat(balance.toFixed(2));
    remainingValue = parseFloat((remainingValue - balance).toFixed(2));
...
export default function WalletPage({ params }: WalletPageProps) {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      const generatedTotalValue = parseFloat((Math.random() * (maxPrice - minPrice) + minPrice).toFixed(2));
      setTotalValue(generatedTotalValue);
      console.log('Generated total value:', generatedTotalValue);
      
      const balances = generateTokenBalances(generatedTotalValue, tonTokens);
